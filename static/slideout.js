...
function Slideout(options) {
  ...
  this._padding2 = parseInt(options.padding2, 10) || 0;
  this._orientation = options.side === 'right' ? -1 : 1;
  this._translateTo *= this._orientation;

  this._speed = options.speed || 1;
  this._both = options.both || false;
  this._limit = options.limit || false;
...
Slideout.prototype.open = function() {
                                                                                                            //itemsmenu_container.open()
  console.log('opened!!');
  var self = this;
  this.emit('beforeopen');
  if (html.className.search('slideout-open') === -1) { html.className += ' slideout-open'; }
  this._setTransition();
  this._translateTo = this._orientation * this._padding + (this._padding2 * self._opened);
  this._translateXTo(this._translateTo);
  if (this._opened) { this._opened2 = true }
  this._opened = true;
  setTimeout(function() {
...
Slideout.prototype.close = function() {
  //                                                                                    if(this._opened2==false){itemsmenu_container.close()}
  console.log('closed!!');
  var self = this;
  if (!this.isOpen() && !this._opening) { return this; }
  this.emit('beforeclose');
  this._setTransition();
  this._opened2 ? this._translateXTo(this._padding): this._translateXTo(0);
  this._opened2 ? this._opened2 = false: this._opened = false;
  if( !this._opened && !this._opened2 ){ //BARA OM DEN STÄNGS HELT TRIGGAS DETTA:
    setTimeout(function() {
...
Slideout.prototype.isOpen2 = function() {
  return this._opened2; 
};
...
Slideout.prototype._initTouchEvents = function() {
  ...
  this._onTouchEndFn = function() {
    if (self._moved) {
      console.log(Math.abs(self._currentOffsetX));
  ...
  this._onTouchMoveFn = function(eve) {
    ...
    var dif_x = (eve.touches[0].clientX - self._startOffsetX ) * self._speed + (self._padding * (self._opened)) ;
    ...
    if(self._both && self._opening){ self._orientation = dif_x < 0 ? -1 : 1; }

    if (Math.abs(dif_x) > 2) {
      self._opening = true;
      //if (dif_x > 0){self._orientation = 1 } else { self._orientation = -1 } //KONVERTERAR RIKTINGEN BEROENDE PÅ INITIAL TOUCH

      var oriented_dif_x = dif_x * self._orientation;
      if ( self._opened && oriented_dif_x > 0 ){

      } //"ÖPPNAR" NÄR ÖPPEN..

      if ( !self._opened && oriented_dif_x < 0 ){ return; } //OM STÄNGAR NÄR STÄNGD..
      ...
      self.panel.style[prefix + 'transform'] = self.panel.style.transform = 'translate3d(' + (translateX + (150*self._opened2)) + 'px, 0, 0)';
