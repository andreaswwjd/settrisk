<template>
  <div class="new">
    ...
    <div><button v-on:click="activateGame()" class="lockbtn" v-bind:style="{background: !game.active?'#f1c40f':'#f0d5aa'}" v-html="lockbtnHTML">{{!game.active?'Lås inställningar':'Låst'}}</button></div>
    <div><button v-on:click="pauseToggle()" v-show="game.active" class="pausebtn" v-bind:style="{background: game.paused?'#f1c40f':'indianred'}" v-html="playbtnHTML"></button></div>
    ...
    <div class="plusminusbutton" v-bind:key="key" v-for="(val,key) in game.resurser" v-if="game.resurser">
    ...
    <div class="plusminusbutton" v-bind:class="{inactive: building.inactive}" v-bind:key="building.type" v-for="building in buildingTypes" v-if="buildingTypes">
      <div class="innerbutton" style="left: 0;" v-on:click="removeStarterByggnad(building)">
        <i class="fas fa-minus" style="left: 20px; color: indianred;" ></i>
      </div>
      <div class="innerbutton" style="right: 0;" v-on:click="addStarterByggnad(building)">
        <i class="fas fa-plus" style="right: 20px; color: #4CAF50;"></i>
      </div>
      <div style="display: inline-block;" ><strong> {{building.name}} <span style="float:right">{{starters.byggnader[building.type] || 0 }}st</span></strong></div>
    </div>
...
export default {
  ...
  computed: {
    ...
    starters: function(){
      let resurser = {}
      Object.keys(this.game.resurser).map((restype) => {
      ...
      let byggnader = {}
      Object.keys(this.buildingTypes).map((byggtype) => {
        let nr_of_byggtype = this.game.starters.byggnader.filter(bygg=>bygg.type.replace(' ','_')==byggtype).length
        if(nr_of_byggtype){
          byggnader[byggtype] = nr_of_byggtype;
        }
      })
      let infanteri = {}
      Object.keys(this.armyTypes).map((armytype) => {
        let nr_of_armytype = this.game.starters.infanteri.filter(bygg=>bygg.name.replace(' ','_')==armytype).length
        if(nr_of_armytype){
  ...
  methods: {
    ...
    addStarterByggnad: function(type){// är nu inte type utan en byggnad...
      console.log('Add type %s.',type)
