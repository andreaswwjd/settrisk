    ...
    <g class="boardpiece" v-for="boardpiece in boardgame.bps" v-bind:id="boardpiece.id">
      ...
      <g v-bind:id="'fields_'+boardpiece.id">
        <g v-for="field in boardpiece.fields" class="field" v-bind:type="field.type" v-on:click="selected.tool=='edit'?boardpiece.editField(field):'';">
          <!-- <g v-html="field.svg"> -->
          <polygon v-for="f in field.array" v-bind:points="toTrianglePoints(f)" v-bind:fill="f.fill" v-on:click="selected.tool=='number'? field.number={_pos:f._pos, nr:'?'}:'';"></polygon>
          <!-- </g> -->
      ...
      <g v-bind:id="'buildsites_'+boardpiece.id">
        <g class="buildsite" v-for="buildsite in boardpiece.buildings" v-on:click="selected.tool == 'building' && altPress ? boardpiece.removeBuilding(buildsite) :'';" v-bind:transform="'translate('+buildsite._pos.toX()+', '+buildsite._pos.toY()+')'" style="fill: black; fill-opacity: 0.5; ">
...
export default {
  ...
  beforeMount: function(){
      ...
      return {
        ...
        rotateX: function(shift_x = 0, shift_y = 0){
          if(vm.altPress && vm.selected.tool == 'move'){ 
            var x = (this._x + shift_x - vm.rotation.origin._x)/2;
            var y = (this._y + shift_y - vm.rotation.origin._y)* vm.distY/vm.distX;
            var x_ny = x * Math.cos(vm.rotation.angle) - y * Math.sin(vm.rotation.angle) ;
            this.x_ny = Math.round((x_ny * 2) + vm.rotation.origin._x + vm.translate._x);
            return Math.round((x_ny * 2) + vm.rotation.origin._x + vm.translate._x);
          }else{
            this.x_ny = this._x + shift_x + vm.translate._x;
            return this._x + shift_x + vm.translate._x;
          }
        },
        rotateY: function(shift_x = 0, shift_y = 0){
          if(vm.altPress && vm.selected.tool == 'move'){ 
            var x = (this._x + shift_x - vm.rotation.origin._x)/2;
            var y = (this._y + shift_y - vm.rotation.origin._y)* vm.distY/vm.distX;
            var y_ny = x * Math.sin(vm.rotation.angle) + y * Math.cos(vm.rotation.angle) ;
            this.y_ny = Math.round((y_ny * vm.distX/vm.distY) + vm.rotation.origin._y + vm.translate._y) ;
            return Math.round((y_ny * vm.distX/vm.distY) + vm.rotation.origin._y + vm.translate._y) ;
          }else{
            this.y_ny = this._y + shift_y + vm.translate._y;
            return this._y + shift_y + vm.translate._y;
          }
        }
      }
    }
    ...
    this.boardpieceClass = function(boardpiece = {}){
      ...
      this.setBuilding = function(d){
        var arr = [ 
          new vm._position(d._pos._x-2, d._pos._y), new vm._position(d._pos._x-1, d._pos._y), new vm._position(d._pos._x,d._pos._y), 
          new vm._position(d._pos._x-2, d._pos._y-1), new vm._position(d._pos._x-1,d._pos._y-1), new vm._position(d._pos._x, d._pos._y-1)
        ]
        var a = arr.map((ai,i)=>{
          var match = self.fields.map((fie)=>{return fie.array.find((f)=>{ return ai._x+'-'+ai._y == f.index }) }).filter((v)=>{return v})[0] 
          return match ? {index: ai._x+'-'+ai._y, _pos: ai, flip: i%2==1, refid: match.refid, type: match.type} : {index: ai._x+'-'+ai._y, _pos: ai, flip: i%2==1};
      ...
      this.setStartMotion = function(d){
        if(vm.selected.tool == 'move'){
          vm.translate = { _x: 0, _y: 0 };
          vm.selected._pos = d._pos;
          vm.selected.flip = d.flip;
          vm.rotation.origin = d.flip ? {_x: d._pos._x+1, _y: d._pos._y}: d._pos; 
          vm.rotation.angle = 0;
        }
      }
      ...
      this.moveContent = function(d){
        if(vm.mdown && vm.selected.tool == 'move' && d.flip == vm.selected.flip){
          if(vm.altPress){ 
            vm.rotation.angle = ( d._pos._y-(vm.rotation.origin._y+vm.translate._y) ) * Math.PI*2/6 ;
          }else{
            vm.translate = { _x: d._pos._x-vm.selected._pos._x, _y: d._pos._y-vm.selected._pos._y };
          }
          // self.updateAll()
        }
      }
      ...
      this.translateMotion = function(d){
        if(vm.selected.tool == 'move' && (vm.translate._x || vm.translate._y || vm.rotation.angle) ){
          self.fields = self.fields.map((field)=>{ return self.transformField( field, vm.translate._x, vm.translate._y )})
          self.roads = self.roads.map((road)=>{ return self.transformRoad( road, vm.translate._x, vm.translate._y )});
          self.buildings = self.buildings.map((building)=>{return self.transformBuilding( building, vm.translate._x, vm.translate._y )})
          self.resetTransform()
          // self.updateAll()
        }
      }
      ...
      this.save = function(){
        ...
        var temp = JSON.parse(JSON.stringify(vm.translate));
        vm.translate = {_x: -min_x, _y: -min_y}
        
        var svgxml = document.getElementById(self.id).innerHTML;
        self.svg = '<svg width="'+((max_x-min_x+2)*vm.distX/2+vm.marginX*2)+'" height="'+((max_y-min_y+1)*vm.distY+vm.marginY*2)+'">'+svgxml+'</svg>';
        vm.translate = temp;
        self.updateAll()
      ...
      this.transformField = function( field, transformX, transformY){
        var i = (Math.round(vm.rotation.angle*3/Math.PI)%6+6)%6; // i = 0 to 5 according to angle (both positive and negative).
        var f = i%2; // = true when transformed needs to be fliped.
        field.array = field.array.map((d)=>{
          var tr = d.flip ? vm.rotation.tr_conv.f[i] : vm.rotation.tr_conv.nf[i] ;
          d._pos._x = d._pos.x_ny+tr[0];
          d._pos._y = d._pos.y_ny+tr[1];
          d.index = d._pos.x_ny+'-'+d._pos.y_ny;
          d.flip = f ? !d.flip : d.flip ;
