    ...
    <div style="max-width: 550px; margin: 40px auto;background: white; color: black;">
      ...
      <svg id="canvas" v-bind:height="canvasHeight" v-bind:width="canvasWidth">
          ...
          <g class="boardpiece" v-for="boardpiece in boardgame.bps" v-bind:id="boardpiece.id" v-on:click="boardpieceClick(boardpiece)" v-bind:style="{fillOpacity: currentpiece==boardpiece ? '1' : '0.5' }">
            ...
            <g v-bind:id="'fields_'+boardpiece.id">
              <g v-for="field in boardpiece.fields" class="field" v-bind:type="field.type" v-on:click="selected.tool=='edit'?boardpiece.editField(field):'';">
                <!-- <g v-html="field.svg"> -->
                <polygon v-for="f in field.array" v-bind:points="toTrianglePoints(f)" v-bind:fill="f.fill" v-on:click="selected.tool=='number'? boardpiece.setFieldNumber(f, field):'';"></polygon>
                <!-- </g> -->
                <g class="number" v-if="field.number.nr" style="text-anchor: middle; font-family: 'Helvetica'; font-size: 13px; fill: #606060;">
                  <circle v-bind:cx="field.number._pos.toX()" v-bind:cy="field.number._pos.toY()" r="8.1"/>
                  <text v-bind:transform="'matrix(1 0 0 1 '+field.number._pos.toX()+' '+(field.number._pos.toY()+4.5)+')'" fill="#FFF">{{field.number.nr}}</text>
                </g>
              </g>
            </g>

            <g v-bind:id="'roads_'+boardpiece.id">
              <g class="road" v-for="d in boardpiece.roads" v-on:click="selected.tool == 'road' && altPress ? boardpiece.removeRoad(d) :'';" style="fill: #606060; stroke: #606060; stroke-width: 4;">
                <circle v-bind:cx="d[0].toX()" v-bind:cy="d[0].toY()" r="2" />
                <circle v-bind:cx="d[1].toX()" v-bind:cy="d[1].toY()" r="2" />
                <line v-bind:x1="d[0].toX()" v-bind:y1="d[0].toY()" v-bind:x2="d[1].toX()" v-bind:y2="d[1].toY()" />
              </g>
              <g class="road" v-if="boardpiece.prelRoad[0]" v-for="d in [boardpiece.prelRoad]" v-on:click="selected.tool == 'road' && altPress ? boardpiece.removeRoad(d) :'';" style="fill: #606060; stroke: #606060; stroke-width: 4;">
                <circle v-bind:cx="d[0].toX()" v-bind:cy="d[0].toY()" r="2" />
                <circle v-bind:cx="d[1].toX()" v-bind:cy="d[1].toY()" r="2" />
                <line v-bind:x1="d[0].toX()" v-bind:y1="d[0].toY()" v-bind:x2="d[1].toX()" v-bind:y2="d[1].toY()" />
              </g>
            ...
            <g v-bind:id="'buildsites_'+boardpiece.id">
              <g class="buildsite" v-for="buildsite in boardpiece.buildings" v-on:click="selected.tool == 'building' && altPress ? boardpiece.removeBuilding(buildsite) :'';" v-bind:transform="'translate('+buildsite._pos.toX()+', '+buildsite._pos.toY()+')'" style="fill: black; fill-opacity: 0.5; " v-bind:data="buildsite.fields()">
        ...
        <g id="grid" v-bind:style="{pointerEvents: ['edit','road','building','number','move','hand'].indexOf(selected.tool) != -1 ? 'none' : 'auto'}">
          <polygon v-for="d in mesh" v-bind:points="toTrianglePoints(d)" class="triangle" v-once
        ...
        <g id="griddots" v-bind:style="{display: griddotsDisplay, pointerEvents: ['road','building','move'].indexOf(selected.tool) != -1 ? 'auto' : 'none'}">
          <circle class="dot" v-for="d in dotmesh" v-bind:cx="d._pos.toX()" v-bind:cy="d._pos.toY()" r="0" v-once
            v-on:mousedown="_gridDotMouseDown(d)"
            v-on:mouseover="_gridDotMouseOver(d)"
            v-on:mouseup="_gridDotMouseUp(d)"
            v-on:click="_gridDotClick(d)"></circle>
        </g>
        ...
        <g id="menu">
          ...
          <g id="edit_btn" v-on:click="selectTool(1)" style="transform: translateX(30px);">
            <title>Edit Field</title>
            <use xlink:href="#edit_btn"></use>
          </g>
          <g id="road_btn" v-on:click="selectTool(2)" style="transform: translateX(60px);">
          ...
          <g id="building_btn" v-on:click="selectTool(3)" style="transform: translateX(90px);">
            <title>Create/Edit Building-site</title>
            <path class="st0" d="M22,25H3c-1.7,0-3-1.3-3-3V3c0-1.7,1.3-3,3-3h19c1.7,0,3,1.3,3,3v19C25,23.7,23.7,25,22,25z"/>
          <polygon class="st4" points="17.5,3.8 7.5,3.8 2.5,12.5 7.5,21.2 17.5,21.2 22.5,12.5 "/>
          </g>
          <g id="nr_btn" v-on:click="selectTool(4)" style="transform: translateX(120px);">
            <title>Add numberplate</title>
            <use xlink:href="#nr_btn"></use>
          </g>
          <g id="move_btn" v-on:click="selectTool(5)" style="transform: translateX(200px);">
      ...
      16.2,14.8 "/>
          ...
          <g id="hand_btn" v-on:click="selectTool(6)" style="transform: translateX(230px);">
      ...
      <div id="saved" style="display: inline-block;">
        <div v-for="boardpiece in boardpieces">
          <div v-html="boardpiece.svg"></div>
          <button v-on:click="addBpToGameBoard(boardpiece.id)">Lägg till</button>
          <button v-on:click="editBp(boardpiece.id)">Edit</button></div>
        </div>
      </div>
      <h1>Spelplaner:</h1>
...
export default {
  ...
  methods: {
    ...
    ******************************/
    ...
    _gridDotMouseUp: function(d){
      ...
      if(this.selected.tool == 'move' && (this.currentpiece.translate._x || this.currentpiece.translate._y || this.currentpiece.rotation.angle) ){
    ...
    selectTool: function(i,d){
      ...
      ['road','building','move'].indexOf(this.selected.tool) != -1 ? 
        this.griddotsDisplay = '' : 
        this.griddotsDisplay = 'none' ;
    },

    boardpieceClick: function(bp){
      if( this.selected.tool == 'hand' ){
        this.selectTool(5)
        if(this.currentpiece != bp){
          this.currentpiece = bp;
          this.currentpiece.setStartMotion(bp)
        }
      }
    },

    saveBoard: function(){
      this.boardgame.bps.length > 1 ? this.saveBoardGame() : this.currentpiece.save() ;
    },

    saveBoardGame: function() {
      this.boardgame.bps = this.boardgame.bps.map((bp)=>{ // Update fields on each building.
        bp.fields = bp.fields.map((f)=>{
          // f.svg = f.array.map((t)=>{ return '<polygon points="'+this.toTrianglePoints(t)+'"/>' }).join();
          // f.svg += paintNumberBadge(f);
          f.pos = {x: f.number._pos.toX(1,0), y: f.number._pos.toY(0,0.5)};
          f.occupiedBy = 'none';
          return f;
        })
        // bp.roadsSVG = bp.roads.map((r)=>{
        //   return paintRoads(r);
        // })
        bp.buildings = bp.buildings.map((b)=>{
          // b.fields = b.fieldsFn(),
          // b.svg = paintBuildings(b);
          b.pos = {x: b._pos.toX(), y: b._pos.toY()};
          return b;
        })
        return bp;
      })
      var svgxml = document.getElementById('boardgame').innerHTML;
      this.boardgame.svg = '<svg width="'+this.canvasWidth+'" height="'+this.canvasHeight+'">'+svgxml+'</svg>';
      
      if(this.boardgames.find((b)=>{ return b.id == this.boardgame.id }) != undefined){
        this.boardgames = this.boardgames.map((b)=>{ return b.id == this.boardgame.id ? this.boardgame : b ;})
      }else{
        this.boardgames.push(this.boardgame);
      } 
    },

    addBpToGameBoard: function(id){
      this.currentpiece = this.boardpieces.find((d)=>{ return d.id==id});
      if( this.boardgame.bps.find((d)=>{ return d.id==id}) != undefined && confirm('Brickan verkar redan finnas på arbetsytan. Lägger du till den kommer den gamla bli översparad.')){
        this.boardgame.bps = this.boardgame.bps.map((b)=>{return b.id == id ? this.currentpiece : b ; })
        // updateAllBoards()
      }else{
        this.boardgame.bps.push(this.currentpiece);
        // updateAllBoards()
      }
    },

    editBp: function(id){
      if(this.boardgame.bps.length && confirm('Är du säker? Osparade brickor i arbetsfältet kommer raderas.')){
        this.currentpiece = this.boardpieces.find((d)=>{ return d.id==id});
        this.boardgame.bps = [this.currentpiece];
        // updateFrame()
        // updateAllBoards()
    ...
    newBp: function(id){
      if(this.boardgame.bps.length && confirm('Är du säker? Osparade brickor i arbetsfältet kommer raderas.')){
        this.boardgame = new this.boardgameClass();
        this.boardgame.bps[0] = this.currentpiece = new this.boardpieceClass();
        this.boardpieces.push(this.currentpiece)
        // updateFrame()
        // updateAllBoards()
      }
    }
  ...
  beforeMount: function(){
    ...
    this._position = function(_x = 0, _y = 0) {
      return {
        ...
        toX: function(shift_x = 0, shift_y = 0){
          return this.rotateX(shift_x, shift_y) * vm.distX/2 + vm.marginX ;
        },
        toY: function(shift_x = 0, shift_y = 0){
          return this.rotateY(shift_x, shift_y) * vm.distY + vm.marginY ;
        },
        rotateX: function(shift_x = 0, shift_y = 0, translate = vm.currentpiece.translate, rotation = vm.currentpiece.rotation ){
          if(vm.altPress && vm.selected.tool == 'move'){ 
            var x = (this._x + shift_x - rotation.origin._x)/2;
            var y = (this._y + shift_y - rotation.origin._y)* vm.distY/vm.distX;
            var x_ny = x * Math.cos(rotation.angle) - y * Math.sin(rotation.angle) ;
            this.x_ny = ((x_ny * 2) + rotation.origin._x + translate._x);
            return ((x_ny * 2) + rotation.origin._x + translate._x);
          }else{
            this.x_ny = this._x + shift_x + translate._x;
            return this._x + shift_x + translate._x;
          }
        },
        rotateY: function(shift_x = 0, shift_y = 0, translate = vm.currentpiece.translate, rotation = vm.currentpiece.rotation ){
          if(vm.altPress && vm.selected.tool == 'move'){ 
            var x = (this._x + shift_x - rotation.origin._x)/2;
            var y = (this._y + shift_y - rotation.origin._y)* vm.distY/vm.distX;
            var y_ny = x * Math.sin(rotation.angle) + y * Math.cos(rotation.angle) ;
            this.y_ny = ((y_ny * vm.distX/vm.distY) + rotation.origin._y + translate._y) ;
            return ((y_ny * vm.distX/vm.distY) + rotation.origin._y + translate._y) ;
          }else{
            this.y_ny = this._y + shift_y + translate._y;
            return this._y + shift_y + translate._y;
          }
        }
      }
    }
    ...
    this.boardgameClass = function(){
      this.id = c4()+'_'+c4()+'_'+c4();
      this.bps = []
    }

    this.boardpieceClass = function(boardpiece = {}){
      let self = this;
      ...
      this.translate = { _x:0, _y:0 };
      this.rotation = { origin: {_x:0, _y:0}, angle:0, tr_conv: {
          nf: [[-1,-1],[0,-1],[0,0],[-1,0],[-2,0],[-2,-1]], 
          f: [[-1,0],[-2,0],[-2,-1],[-1,-1],[0,-1],[0,0]]} 
      };
      ...
      this.editField = function(d){
        ...
        vm.selectTool(0, vm.tools[0].fieldTools.find(function(t){return t.type == d.type;}))
      }
        ...
        if(self.current_field.array.find((t)=>{return t.index==d.index}) == undefined ){
          self.current_field.array.push({
            refid: self.current_field.id,
              index: d.index,
              _pos: new vm._position( d._pos._x, d._pos._y ),
              flip: d.flip,
              fill: color,
              type: type
            })
      ...
      this.setFieldNumber = function(d, field){
        let nr = Math.ceil(Math.random()*10+1);
        field.number = {_pos: new vm._position(d._pos._x+1,d._pos._y+0.5), nr: nr==7 ? 12 : nr};
      ...
      this.setRoadStart = function(d){
        vm.altPress ? 'pass' : self.prelRoad=[ new vm._position(d._pos._x, d._pos._y), new vm._position(d._pos._x, d._pos._y)] ;
        self.updateRoads();
      }
      ...
      this.setBuilding = function(d){
        var b = {
          ...
          fields: function(){
            var a = [];
            var hits = {}
            vm.boardgame.bps.map((bp)=>{
              bp.fields.map((field)=>{
                a = a.concat(field.array.map((f)=>{
                  if( f._pos._x >= this._pos._x-2 && f._pos._x <= this._pos._x && 
                      f._pos._y >= this._pos._y-1 && f._pos._y <= this._pos._y ){
                    hits[f._pos._x+'-'+f._pos._y] ? hits[f._pos._x+'-'+f._pos._y] += 1 : hits[f._pos._x+'-'+f._pos._y] = 1;
                    return {bp_id: self.id, refid: f.refid, type: f.type, nr: field.number ? field.number.nr : undefined}
                  }else{
                    return false
                  }
                })).filter(v=>v)
      ...
      this.setStartMotion = function(d){
        if(vm.selected.tool == 'move'){
          this.translate = { _x: 0, _y: 0 };
          vm.selected._pos = d._pos;
          vm.selected.flip = d.flip;
          this.rotation.origin = d.flip ? {_x: d._pos._x+1, _y: d._pos._y}: d._pos; 
          this.rotation.angle = 0;
        }
      }

      this.moveContent = function(d){
        if(vm.mdown && vm.selected.tool == 'move' && d.flip == vm.selected.flip){
          if(vm.altPress){ 
            this.rotation.angle = ( d._pos._y-(this.rotation.origin._y+this.translate._y) ) * Math.PI*2/6 ;
          }else{
            this.translate = { _x: d._pos._x-vm.selected._pos._x, _y: d._pos._y-vm.selected._pos._y };
          }
          // self.updateAll()
        }
      }
      ...
      this.translateMotion = function(d){
        if(vm.selected.tool == 'move' && (self.translate._x || self.translate._y || self.rotation.angle) ){
          self.fields = self.fields.map((field)=>{ return self.transformField( field, self.translate._x, self.translate._y )})
          self.roads = self.roads.map((road)=>{ return self.transformRoad( road, self.translate._x, self.translate._y )});
          self.buildings = self.buildings.map((building)=>{return self.transformBuilding( building, self.translate._x, self.translate._y )})
          self.resetTransform()
          // self.updateAll()
        }
      }
      ...
      this.resetTransform = function(){
        this.translate = { _x: 0, _y: 0 };
        this.rotation.origin = { _x: 0, _y: 0 };
        this.rotation.angle = 0;
      }
      ...
      this.save = function(){
        ...
        var temp = JSON.parse(JSON.stringify(this.translate));
        this.translate = {_x: -min_x, _y: -min_y}
        
        var svgxml = document.getElementById(self.id).innerHTML;
        var svgwidth = ((max_x-min_x+2)*vm.distX/2+vm.marginX*2);
        var svgheight = ((max_y-min_y+1)*vm.distY+vm.marginY*2);
        var svgx = (min_x)*vm.distX/2;
        var svgy = (min_y)*vm.distY;
        self.svg = '<svg width="'+svgwidth+'" height="'+svgheight+'" viewBox="'+svgx+' '+svgy+' '+svgwidth+' '+svgheight+'">'+svgxml+'</svg>';
        this.translate = temp;

        if(vm.boardpieces.find((b)=>{ return b.id == self.id }) != undefined){
          vm.boardpieces = vm.boardpieces.map((b)=>{ return b.id == self.id ? new vm.boardpieceClass(vm.currentpiece) : b ;})
        }else{
          vm.boardpieces.push(new vm.boardpieceClass(vm.currentpiece));
        }
      }

      this.transformField = function( field, transformX, transformY){
        var i = (Math.round(this.rotation.angle*3/Math.PI)%6+6)%6; // i = 0 to 5 according to angle (both positive and negative).
        var f = i%2; // = true when transformed needs to be fliped.
        field.array = field.array.map((d)=>{
          var tr = d.flip ? this.rotation.tr_conv.f[i] : this.rotation.tr_conv.nf[i] ;
        ...
        if(field.number._pos){ 
          field.number._pos._x = field.number._pos.x_ny 
          field.number._pos._y = field.number._pos.y_ny 
        }
        return field
      }

      this.transformRoad = function( road, transformX, transformY ){
        road = road.map((d)=>{
          d._x = d.x_ny
          d._y = d.y_ny
          return d
      ...
      this.transformBuilding = function( building, transformX, transformY ){
        // var i = (Math.round(this.rotation.angle*3/Math.PI)%6+6)%6; // i = 0 to 5 according to angle (both positive and negative).
        // var f = i%2; // = true when transformed needs to be fliped.
        ...
        //   return d
        // });
        building._pos._x = building._pos.x_ny 
        building._pos._y = building._pos.y_ny
        return building
      }
  ...
  mounted: function(){
    ...
    // Init
    this.boardgame = new this.boardgameClass();
    this.boardgame.bps[0] = this.boardpieces[0] = this.currentpiece = new this.boardpieceClass();
    // updateFrame()
    // updateAllBoards()
