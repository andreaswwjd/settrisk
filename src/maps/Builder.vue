<template>
  <div id="builder">
    <div style="max-width: 650px; margin: 40px auto;background: white; color: black;">
      ...
      <svg id="canvas" v-bind:height="canvasHeight" v-bind:width="canvasWidth" v-bind:class="selected.tool+'_btn'">
        ...
        <g id="grid" v-bind:style="{pointerEvents: gridPointerEvents}">
          <polygon v-for="d in mesh" v-bind:points="d.t.join(' ')" class="triangle" v-once
        ...
        <g id="griddots" v-bind:style="{display: griddotsDisplay, pointerEvents: griddotsPointerEvents}">
          <circle class="dot" v-for="d in mesh" v-bind:cx="d.p?d.p[0]:0" v-bind:cy="d.p?d.p[1]:0" r="0" v-once
      ...
      <div id="saved" style="display: inline-block;">
        <div v-for="bp in boardpieces" :key="bp.id">
          <svg v-bind:height="canvasHeight" v-bind:width="canvasWidth"  style="display: inline-block; pointer-events: none">
          ...
          <button v-on:click="boardpiece = bp">Edit</button>
          <button v-on:click="remove(boardpieces,bp)">Delete</button>
          <textarea cols="40" rows="5">{{JSON.stringify(bp)}}</textarea>
        </div>
      </div>
      <h1>Spelplaner:</h1>
      <div id="boardgames" style="display: inline-block;">
        <div v-for="bg in boardgames" :key="bg.id">
          ...
          <button v-on:click="remove(boardgames,bg)">Delete</button>
  ...
  data () {
    ...
    return {
      ...
      fieldTools: [
        ...
        {name:'gräs', tool: 'field', type: 'gräs', color: '#75B96B'}
      ],
      tools: [
        {name:'new', tool: 'new'}, 
        {name:'edit', tool: 'edit'}, 
  ...
  methods: {
    ...
    saveBoard: function(){
      if( this.mode == 'boardpiece' ){ // If just 1 boardpiece it saves boardpiece
        this.update( this.boardpieces, this.boardpiece )
      }else if( this.mode == 'boardgame' ){ // If multiple boardpieces it saves boardgame
        this.update( this.boardgames, this.boardgame )
      }
    },
    newBp: function(){
      this.saveBoard()
